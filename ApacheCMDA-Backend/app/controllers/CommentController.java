...
@Named
@Singleton
public class CommentController extends Controller implements IController {
    ...
    public Result addReply() {
        ...
        StrategyContext strategyComment = new StrategyContext(comment);
        if(comment==null){
        ...
        StrategyContext strategyReply = new StrategyContext(savedReply);
        List<Reply> replyList = comment.getReplies();
        replyList.add(reply);
        comment.setReplies(replyList);
        commentRepository.save(comment);

        return ok(new Gson().toJson(strategyReply.strategyGetId()));
    }
