...
@Named
@Singleton
public class DatasetController extends Controller implements IController {
    ...
    public Result updateDatasetById(long id) {
		...
		try {
			...
			StrategyContext strategyDataset = new StrategyContext(savedDataset);
			System.out.println("Dataset updated: "+ strategyDataset.strategyGetId());
			System.out.println(strategyDataset.strategyToString());
			return created("Dataset updated: "+ strategyDataset.strategyGetId());
		} catch (PersistenceException pe) {
			pe.printStackTrace();
