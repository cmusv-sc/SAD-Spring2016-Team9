...
@Named
@Singleton
public class DatasetLogController extends Controller implements IController {
	...
	public Result addDatasetLog() {
		...
		try {
			...
			StrategyContext strategyDatasetLog = new StrategyContext(saveddatasetLog);
			System.out.println("DatasetLog saved: "+ strategyDatasetLog.strategyGetId());
			System.out.println(strategyDatasetLog.strategyToString());
			return created(new Gson().toJson(new StrategyContext(datasetLog).strategyGetId()));
    ...
    public Result updateDatasetLogById(long id) {
		...
		try {
			...
			DatasetLog savedDatasetLog = datasetLogRepository.save(datasetLog);
			StrategyContext strategyDatasetLog = new StrategyContext(savedDatasetLog);
			System.out.println("DatasetLog updated: "+ strategyDatasetLog.strategyGetId());
			return created("DatasetLog updated: "+ strategyDatasetLog.strategyGetId());
		} catch (PersistenceException pe) {
			pe.printStackTrace();
