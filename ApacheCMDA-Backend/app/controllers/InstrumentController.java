...
@Named
@Singleton
public class InstrumentController extends Controller implements IController {
	...
	public Result addInstrument() {
		...
		try {
			...
			StrategyContext strategyInstrument = new StrategyContext(savedInstrument);
			System.out.println("Instrument saved: "+ strategyInstrument.strategyGetId());
			System.out.println(strategyInstrument.strategyToString());
			return created(new Gson().toJson(new StrategyContext(instrument).strategyGetId()));
    ...
    public Result updateInstrumentById(long id) {
		...
		try {
			...
			Instrument savedInstrument = instrumentRepository.save(instrument);
			StrategyContext strategyInstrument = new StrategyContext(savedInstrument);
			System.out.println("Instrument updated: "+ strategyInstrument.strategyToString());
			return created("Instrument updated: "+ strategyInstrument.strategyToString());
		} catch (PersistenceException pe) {
			pe.printStackTrace();
