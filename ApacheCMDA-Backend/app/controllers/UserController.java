...
public class UserController extends Controller implements IController {
	...
	public Result deleteUserByUserNameandPassword(String userName, String password) {
		try {
			...
			StrategyContext strategyUser = new StrategyContext(user);
			if (user.getPassword().equals(password)) {
				System.out.println("User is deleted: " + strategyUser.strategyGetId());
				System.out.println(strategyUser.strategyToString());
    ...
    public Result userUnfollow(Long userId, Long followeeId){
        try{
			...
			for(User u : followers) {
				if(u.getId()==strategyUser.strategyGetId()) {
					followers.remove(u);
				}
			}
			followee.setFollowers(followers);
	...
	public Result acceptFriendRequest(Long receiverId, Long senderId) {
		try {
			...
			User sender = userRepository.findOne(senderId);
			StrategyContext strategySender = new StrategyContext(sender);
			if(receiverId==null){
			...
			for(User s : reqSenders) {
				if(s.getId() == strategySender.strategyGetId()) {
					flag = true;
					reqSenders.remove(s);
	...
	public Result deleteFriend(Long userId, Long friendId) {
		...
		StrategyContext strategyFriend = new StrategyContext(friend);
		if(friend==null){
		...
		Set<User> friends = user.getFriends();
		for(User f: friends) {
			if(f.getId()==strategyFriend.strategyGetId()) {
				friends.remove(f);
			}
		}
