...
@Named
@Singleton
public class WorkflowController extends Controller implements IController {
...
//        return ok(erro);
    ...
    public Result addComment(){
        try{
            ...
            StrategyContext strategyComment = new StrategyContext(savedComment);
            List<Comment> list = workflow.getComments();
            list.add(comment);
            workflow.setComments(list);
            workflowRepository.save(workflow);
            System.out.println(strategyComment.strategyToString());
            return ok(new Gson().toJson(strategyComment.strategyGetId()));
        } catch (Exception e){
            e.printStackTrace();
    ...
    public Result getByTag(String tagString) {
        try {
            ...
            Tag tag = tagRepository.findByTag(tagString);
            StrategyContext strategyTag = new StrategyContext(tag);
            if(tag==null) {
                System.out.println("Tag doesn't exist");
                return ok();
            } else {
                System.out.println(strategyTag.strategyToString());
            }

            Long tagId = strategyTag.strategyGetId();
